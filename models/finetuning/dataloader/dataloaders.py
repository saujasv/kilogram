...
from .preprocessor import _get_preprocessor
...
def load_filenames(file_to_anns, is_whole_image):
    ...
    for filename, anns in file_to_anns.items():  # take the ith annotation
        ...
        for i, ann in enumerate(anns):
            ...
            pc = (
                ann.count(",") + 1
            )  # dog#head#tail has 2 parts // a dog with a head, and a body has 2 parts

            images_n.append(image_filename)  # ['page1-1_2.png', ...]
            texts_n.append(
                ann
            )  # [dog#head#tail, ...] // [a dog with a head, and a body, ...]
...
class TrainingSampler(Sampler):
    ...
    def __init__(
        self, batch_size, tangrams_n, texts_n, tangrams_set, max_len, parts_count
    ):
        ...
        self._pc2tangrams2idx: Dict = (
            {}
        )  # parts count to (unique) tangrams to a list indices
    ...
    def _reset_batches(self):  # shuffling indices while fulfilling constraints
        ...
        for p in tqdm(
            self._parts2idx.keys()
        ):  # for a certain parts count # constraint [3]
            print("part count: ", p)
            available_idxs = self._parts2idx[p].copy()
            random.shuffle(
                available_idxs
            )  # shuffle the order of examples having the same number of parts.

            available_tangrams2idx = copy.deepcopy(
                self._pc2tangrams2idx[p]
            )  # dict of tangrams to idxs of the same # of parts, remove the tangram when annotations are all picked
            ...
            idx_to_remove = (
                []
            )  # idx already selected for batch after an iteration of available_idx

            while (
                len(available_tangrams2idx) >= self._batch_size
            ):  # there are at least 10 different tangrams/idxs to choose from
                for (
                    example_idx
                ) in available_idxs:  # loop thru all idxs of same # of parts
                ...
                if len(available_tangrams2idx) < self._batch_size:
                    print(
                        "move to next parts count: < 10 different tangrams to choose from"
                    )
                    break
                ...
                if len(idx_to_remove) == 0:
                    print("not selected: ", available_tangrams2idx)
                    print(
                        "move to next parts count: none of the available tangrams satisfies constraints"
                    )
                    break
                ...
                idx_to_remove = (
                    []
                )  # removed from available idxs, enter next loop through all available examples
...
class ValidationDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_file = self.images_n[idx]
        image_path = (
            os.path.join(self.image_path, image_file) + ".png"
            if not image_file.endswith(".png")
            else os.path.join(self.image_path, image_file)
        )
        encodings = self._preprocessor.preprocess(image_path, self.texts[idx])
...
def _get_dataloaders(dpathes: Dict, args: argparse.ArgumentParser) -> Dict:
    ...
    (
        ...
        train_max_len,
        train_parts_count,
    ) = load_filenames(file_to_anns_train, dpathes["is_whole_image"])
    ...
    if args.overfit_debug:
        ...
        dstrain = TrainingDataSet(
            dpathes["train_image_path"],
            train_images_n,
            train_texts_n,
            train_preprocessor,
        )
        ...
        dlval = DataLoader(
            dsval, batch_size=args.batch_size, shuffle=False, drop_last=True
        )
        ...
        dltrain = DataLoader(
            dstrain, batch_size=args.batch_size, shuffle=True, drop_last=True
        )
        dsval = ValidationDataSet(
            dpathes["val_image_path"], val_data, test_preprocessor
        )
        dlval = DataLoader(
            dsval, batch_size=args.batch_size, shuffle=False, drop_last=True
        )
        ...
        sptrain = TrainingSampler(
            batch_size=args.batch_size,
            tangrams_n=train_tangrams_n,
            texts_n=train_texts_n,
            tangrams_set=train_tangrams_set,
    ...
    loaders = {"train_loader": dltrain, "val_loader": dlval}
    return loaders
